...
(function () {
  ...
  var _serverBase = _selfBase() || "https://dot-games-host.vercel.app";
  var CFG = {
    endpoint: _serverBase + "/api/tts",
    maxLen: 190,
    rate: 1.05,
    pitch: 1.0,
  ...
  var enabled = true;
  var lang = "ko";
  var koVoice = null, enVoice = null, jaVoice = null, esVoice = null;
  ...
  function pickExtra(lg){
    var vs = voices(); if (!vs.length) return;
    var prefix = lg === 'ja' ? 'ja' : 'es';
    var match = function(v){ return v.lang && v.lang.toLowerCase().indexOf(prefix) === 0; };
    var voice = vs.find(function(v){ return match(v) && v.localService && !nov(v, EN_NOV); })
      || vs.find(function(v){ return match(v) && !nov(v, EN_NOV); }) || vs.find(match);
    if(lg === 'ja') jaVoice = voice || jaVoice; else esVoice = voice || esVoice;
  }
  if (hasSS()) {
    try {
      pickKo(); pickEn(); pickExtra('ja'); pickExtra('es');
      window.speechSynthesis.onvoiceschanged = function () { pickKo(); pickEn(); pickExtra('ja'); pickExtra('es'); };
      var _vt = 0, _vi = setInterval(function () { pickKo(); pickEn(); pickExtra('ja'); pickExtra('es'); if ((koVoice && enVoice) || ++_vt > 40) clearInterval(_vi); }, 120);
    } catch (e) {}
  }
  ...
  function applyVoice(u, lg){
    ...
    else if (lg === 'ja') { u.lang = 'ja-JP'; if (!jaVoice) pickExtra('ja'); if (jaVoice) u.voice = jaVoice; }
    else if (lg === 'es') { u.lang = 'es-ES'; if (!esVoice) pickExtra('es'); if (esVoice) u.voice = esVoice; }
  ...
  function playServer(queue, tok, lg, onEnd, isRetry){
    ...
    var seg = queue[0];
    var url = CFG.endpoint + '?q=' + encodeURIComponent(seg) + '&tl=' + (['ko','en','ja','es'].indexOf(lg) >= 0 ? lg : 'ko');
  ...
  function speak(text, opts){
    ...
    var lg = (['ko','en','ja','es'].indexOf(opts.lang) >= 0) ? opts.lang : lang;
    _tok++; var tok = _tok;
    if (_cur) { try { _cur.pause(); _cur.src = ''; } catch (e) {} _cur = null; }
    if (hasSS()) { try { window.speechSynthesis.cancel(); } catch (e) {} }
  ...
  window.TW_TTS = {
    speak: speak,
    stop: stop,
    setLang: function (lg) { if (['ko','en','ja','es'].indexOf(lg) >= 0) lang = lg; },
